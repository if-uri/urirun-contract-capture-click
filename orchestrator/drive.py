#!/usr/bin/env python3
# Part of the ifURI solution — ORCHESTRATOR / brama międzyprocesowa (handoff CZĘŚCIOWY).
"""Wiąże dwa procesy po HTTP wspólnym kontraktem, demonstrując handoff CZĘŚCIOWY:

  1. POST producent  →  koperta screen/query/capture (z fullSize: [int])
  2. krawędź WIRES (capture→click: sw←fullSize.0, sh←fullSize.1) → częściowe wejście {sw,sh}
  3. consumer_input_check → mode=PARTIAL (wire nie niesie x,y — te z kroku locate)
  4. dołóż x,y (osobny krok), POST konsument  →  koperta abs/command/click
  5. zwaliduj odpowiedź; exit 0/1

Procesy nie współdzielą obiektu Pythona — tylko JSON i TEN SAM contracts.json."""
...
PROD_ROUTE = "screen/query/capture"
CONS_ROUTE = "abs/command/click"
...
def main() -> int:
    ...
    if code == 0:
        print("  częściowy handoff przez krawędź ['int']-listy, jeden kontrakt, dwa języki: OK")
    return code
