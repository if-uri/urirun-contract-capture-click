...
//
// Konsument w Go dla abs/command/click. Czyta TEN SAM contracts.json co producent Python,
// egzekwuje tę samą bramę wejścia/wyjścia i odpowiada na ten sam transport HTTP — różni się
// TYLKO językiem. Dowód: kontrakt jest niezależny od języka; orchestrator nie wie, czy trafi
// na Python czy Go, bo widzi tylko JSON zgodny z out-schematem.
package main
...
func typeOK(tok string, v interface{}) bool {
	...
	case "str":
		...
		return ok
	case "int":
		// po JSON: float64; w kopercie zbudowanej w Go: natywny int — akceptuj oba
		switch n := v.(type) {
		case float64:
...
// ── ciało handlera ──────────────────────────────────────────────────────────
func numOr(p map[string]interface{}, k string) int {
...
func clickHandler(p map[string]interface{}) map[string]interface{} {
	return map[string]interface{}{
		"ok": true, "connector": "capture-click-go", "action": "click-abs",
		"screen": []interface{}{numOr(p, "sw"), numOr(p, "sh")},
		"did":    fmt.Sprintf("click@(%d,%d)", numOr(p, "x"), numOr(p, "y")),
	}
}
